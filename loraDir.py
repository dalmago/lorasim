...
"""
 ...
 OUTPUT
    ...
    of values for nodes, collisions, transmissions and total energy spent. The two
    colliding message may still pass depending on the relative timing and difference
    in receive power.
...
def frequencyCollision(p1,p2):
    if (abs(p1.freq-p2.freq)<=120 and (p1.bw==500 or p2.bw==500)): # TODO: verify this line. p2.freq or p2.bw?
...
#
class myNode:
    def __init__(self, nodeid, bs, period, packetlen, x=None, y=None, sf_bw=None):
        self.nodeid = nodeid
        self.period = period
        self.bs = bs
        self.x = 0
...
#
class myPacket:
    def __init__(self, nodeid, plen, distance, sf_bw=None):
        global Ptx
        global gamma
        global d0
        global var
...
def run(exp, sf_bw=None, nNodes=None, queue=None):
    ...
    if os.path.isfile("nodes_%s.csv" %(nrNodes)):
        with open ("nodes_%s.csv" %(nrNodes), "r") as f:
            csv = f.readlines()
            ...
            for line in csv[1:]:
                ...
                values = line.split(',')
                if sf_bw is not None:
                    sb = sf_bw[int(values[0])]
                else:
                    sb = None
                node = myNode(int(values[0]), bsId, avgSendTime, 20, float(values[1]), float(values[2]), sb)
                nodes.append(node)
